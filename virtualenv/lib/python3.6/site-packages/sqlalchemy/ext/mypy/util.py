...
class DeclClassApplied:
    ...
    def serialize(self) -> JsonDict:
        return {
            ...
            "mapped_attr_names": [
                (name, type_.serialize()) for name, type_ in self.mapped_attr_names
            ],
...
def _unbound_to_instance(api: SemanticAnalyzerPluginInterface, typ: Type) -> Type:
    ...
    if (
        ...
        return Instance(
            bound_type,
            [
                _unbound_to_instance(api, arg) if isinstance(arg, UnboundType) else arg
                for arg in typ.args
            ],
        )
    else:
...
def _info_for_cls(cls: ClassDef, api: SemanticAnalyzerPluginInterface) -> TypeInfo:
