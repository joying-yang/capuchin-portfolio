...
class Join(roles.DMLTableRole, FromClause):
    ...
    def _populate_column_collection(self):
        ...
        self._columns._populate_separate_keys((col._key_label, col) for col in columns)
...
class Select(
    ...
    def _generate_columns_plus_names(self, anon_for_dupe_key):
        ...
        if self._label_style is LABEL_STYLE_NONE:
            ...
            return [(None, c, c in same_cols or same_cols.add(c)) for c in cols]
        else:
            names = {}

            use_tablename_labels = self._label_style is LABEL_STYLE_TABLENAME_PLUS_COL
    ...
    def _generate_fromclause_column_proxies(self, subquery):
        ...
        tablename_plus_col = self._label_style is LABEL_STYLE_TABLENAME_PLUS_COL
        ...
        for name, c, repeated in self._generate_columns_plus_names(False):
            if c._is_text_clause:
                ...
                key = c._proxy_key
            prox.append(
                c._make_proxy(subquery, key=key, name=name, name_is_truncatable=True)
            )
